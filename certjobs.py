...
def renew_device_certs():
    ...
    command_list = ["curl", "-k", "-u", f"{connection_args_object.service_account_username}:{connection_args_object.adn_vip_password}", "-X", "POST", "-H", "Content-Type:application/json", url_string]
    command_string = " ".join(command_list)
    try:
        # If shell=True, on POSIX the executable argument specifies a replacement shell for the default /bin/sh.
        completed_process_object = subprocess.run(command_string, shell=True, capture_output=True, executable='/bin/sh')
        ...
        output_string = completed_process_object.stdout.decode(encoding='UTF-8').strip().replace("\n", " ")
        print(f"Process returned success code {completed_process_object.returncode} and standard out: {output_string}")
        ...
        print(f"Exception: command returned {exception_object.output} with exit status {exception_object.returncode}")
    except:
...
def requests_cert_retry():
    ...
    try:
        response_object = requests.post(url,
                                        ...
                                        auth=requests.auth.HTTPBasicAuth(connection_args_object.service_account_username, connection_args_object.adn_vip_password),
        )
        ...
        print(f"venafi_retry job received response code {response_object.status_code} and text: {response_object.text}")
