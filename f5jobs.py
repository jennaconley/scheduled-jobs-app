...
def create_backup_file_on_f5(f5_hostname, f5_username, f5_password):
    ...
    try:
        ...
        print(f"{type(error_object)} F5 web API call failed to create new .ucs backup on {f5_hostname}. Response: {response_object.status_code} - {response_object.text}. ({error_object})")
        backup_filename = None
    except:
        exception_type, exception_value, exception_traceback = sys.exc_info()
        traceback_list = traceback.format_exception(exception_type, exception_value, exception_traceback)
        traceback_string = " ".join(traceback_list)
        print(f"{exception_type}: attempt to create new .ucs backup for {f5_hostname} failed. Traceback: {traceback_string}")
        backup_filename = None
    else:
...
def scp_with_paramiko_no_shell(vm_host, vm_username, vm_password, f5_username, f5_password, f5_host, backup_filename):
    """
    ...
    try:
        ...
        client.connect(f"{vm_host}.central.widgets.com",
                        ssh_port,
                        f5_username,
                        vm_password,
                        look_for_keys=False,
        ...
        for loop in range(0, 1200):
            if stdout.channel.recv_ready():
                ...
                standard_out_string = standard_out_string + stdout_character.decode(encoding='UTF-8')
                ...
                if len(stdout_character) == 0:
                    break
            time.sleep(0.01)
        compressed_stdout = standard_out_string.strip().replace('\n', ' ').replace('\r', ' ')
        ...
        output_string = stdout_string.decode(encoding='UTF-8').strip().replace("\n", " ")
        ...
        print(f"Attempt to authenticate to host {vm_host} for scp failed: {sys.exc_info()[0]}")
        exit_code = 1
    except paramiko.SSHException as ssh_exception_object:
        print(f"An exception occurred while attempting to scp a file from F5 host {vm_host}: {str(ssh_exception_object)}")
        exit_code = 1
    except:
        exception_type, exception_value, exception_traceback = sys.exc_info()
        print(f"An exception occurred while attempting to scp a file from F5 host {vm_host}: {exception_type}")
        traceback_list = traceback.format_exception(exception_type, exception_value, exception_traceback)
...
def remove_backup_file_from_f5(host, f5_username, f5_password, backup_filename):
    ...
    try:
        ...
        ssh_connection.connect(f"{host}-central.widgets.com",
                                ssh_port,
                                f5_username,
                                f5_password,
                                timeout=6000,
        ...
        print(f"An exception occurred during ssh to host {host}: {str(ssh_exception_object)}")
        exit_code = 1
    except:
        exception_type, exception_value, exception_traceback = sys.exc_info()
...
def list_backup_files_on_vm(hostname, vm_username, vm_password):
    ...
    print(f"Connecting to {hostname} as {vm_username} to send command: {command_string}")
    try:
        ssh_connection = paramiko.SSHClient()
        ssh_connection.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        ssh_connection.connect(f"{hostname}.central.widgets.com", ssh_port, vm_username, vm_password)
        stdin, stdout, stderr = ssh_connection.exec_command(command_string)
        stdout_string = stdout.read()
        output_string = stdout_string.decode(encoding='UTF-8').strip().replace("\n", " ")
        ...
        print(f"An exception occurred during ssh to host {hostname}:", str(ssh_exception_object))
        exit_code = 1
    finally:
        ssh_connection.close()
    return(exit_code)
...
def remove_backup_files_from_vm(hostname, vm_username, vm_password):
    ...
    command_string = f"find /home/{vm_username}/backups/*.ucs -mtime +2 -exec rm {{}} \;"
    print(f"Connecting to {hostname} as {vm_username} to send command: {command_string}")
    try:
        ssh_connection = paramiko.SSHClient()
        ssh_connection.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        ...
        output_string = stdout_string.decode(encoding='UTF-8').strip().replace("\n", " ")
        exit_code = stdout.channel.recv_exit_status()
        print(f"Command '{command_string}' to {hostname} produced exit code {exit_code} and output: {output_string}")
    except paramiko.ssh_exception.AuthenticationException:
        print(f"Authentication to {hostname} failed: {sys.exc_info()[0]}")
        exit_code = 1
    except paramiko.SSHException as ssh_exception_object:
...
def send_metric(one_or_zero, f5_host):
    ...
    date_and_time_string = datetime.datetime.now().strftime("%H:%M on %m/%d/%Y")
    ...
    with warnings.catch_warnings():
        ...
        try:
            response = requests.post(url, headers=headers, data=request_body, timeout=10, verify=False)
            response.raise_for_status()
            response.encoding = "UTF-8"
            print(f"BACKUP UPDATE: Sent {value_fields} metric for {f5_host}: {response.status_code}: {response.text}")
        except requests.exceptions.RequestException as error_object:
            print(f"BACKUP UPDATE: POST Request to metrics front door for {f5_host} failed. {type(error_object)}: {error_object}")
...
def create_f5_remote_backups():
    ...
    f5_hostname_list = get_ttb_netbox_hosts(args_object.netbox_token)
    print(f"Found {len(f5_hostname_list)} hosts in Netbox: {', '.join(f5_hostname_list)}")

    # Tell all the F5 load balancers to make backup files
    f5_backup_filename_dict = {}
    for f5_host in f5_hostname_list:
        backup_filename = create_backup_file_on_f5(f5_host, args_object.f5_username, args_object.f5_password)
        f5_backup_filename_dict[f5_host] = backup_filename

    # Wait for the back-up file/s to be prepared
    sleep_seconds_int = 500
    print(f"Pausing for {sleep_seconds_int} seconds while the F5 load balancers create their back-up files")
    time.sleep(500)
    ...
    for f5_host, backup_filename in f5_backup_filename_dict.items():
        ...
        exit_code = scp_with_paramiko_no_shell(vm_host,
        ...
        if exit_code != 0:
            ...
            send_metric("0", f5_host)
            failure_list.append(f5_host)
            remove_backup_file_from_f5(f5_host, args_object.f5_username, args_object.f5_password, backup_filename)
            continue

        print(f"SCP from F5 {f5_host} returned success code {exit_code}.")
        exit_code = remove_backup_file_from_f5(f5_host, args_object.f5_username, args_object.f5_password, backup_filename)
        if exit_code != 0:
            print(f"File removal command sent to f5 {f5_host} returned error code {exit_code}.")
            # Zero is failure in the Grafana dashboard
            send_metric("0", f5_host)
            failure_list.append(f5_host)
        else:
            print(f"File removal command sent to f5 {f5_host} returned success code {exit_code}.")
    ...
    remove_backup_files_from_vm(vm_host, args_object.f5_username, args_object.f5_password)

    print(f"Completed F5 back-up jobs with {len(success_list)} of {len(f5_hostname_list)} successful.")
