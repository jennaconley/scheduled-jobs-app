...
def convert_central_to_utc(hour_central_time, minute_central_time, function_name):
    ...
    job_time_no_timezone_datetime_object = datetime(
        current_datetime_object.year,
    ...
    job_central_datetime_object = timezone_object_central_time.localize(
        job_time_no_timezone_datetime_object
    )
    ...
    print(
        f"Preparing to schedule {function_name} to run at {central_string} Central Time (UTC {utc_string}). "
    )
...
def run_scheduled_jobs():
    ...
    for job_function in list_of_functions:
        ...
        job_utc_datetime_object = convert_central_to_utc(
            job_hour_central_time,
            job_minute_central_time,
            job_function.__name__,
        )
        # Add job to the scheduler object
        scheduler_object.add_job(
            job_function,
            "cron",
